...
try:
    ...
    from docs.labs.domain_graph.graph import DomainGraph
    ...
    from ego_networks.docs.labs.domain_graph.graph import DomainGraph
...
def main():
    ...
    # ]
    # print(f"Sample tokens: {domains[:5]}, Total tokens: {len(domains)}")

    # embeddings = get_embeddings(models[0], domains)

    # similar_tokens = get_similarities(embeddings, domains)
    ...
    G = DomainGraph(nodes=domains).create()
    node_diffusion_grades = G.get_diffusion_grades(content)
    ...
    (
        ...
        graph=G, diffusion_grades=node_diffusion_grades
    ).create()
